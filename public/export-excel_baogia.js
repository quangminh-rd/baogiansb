document.getElementById('export-excel_baogia').addEventListener('click', async function () {
    ...
    try {
        ...
        orderItems.forEach((item, index) => {
            ...
            row.getCell(13).value = parseFloat(formatNumber(item.mucVAT));
        });
        ...
        if ((orderDetails.phiVanchuyenlapdatnpp === "" || orderDetails.phiVanchuyenlapdatnpp === "0") && orderDetails.donviPhutrach === "BP. BH1" && orderDetails.phuongThucban == "Bán lẻ") {
            worksheet.getCell('A520').value = '2. Giá trên đã bao gồm phí vận chuyển, lắp đặt tại nội thành Hà Nội.';
        ...
        if (orderDetails.donviPhutrach === "BP. BH1" && orderDetails.phuongThucban === "Bán đại lý") {
            ...
            worksheet.getCell('A525').value = ' - Bảo hành 2 năm phụ kiện lắp đồng bộ với cửa, tay nắm, bánh xe, ổ khóa, động cơ, điều khiển, phụ kiện nhựa, …';
            worksheet.getCell('A526').value = ' - Bảo hành 2 năm sản phẩm Cửa xếp rèm tổ ong; Cửa xếp 2 trong 1/ xếp xích 2 trong 1 kết hợp lưới và rèm.';
            worksheet.getCell('A527').value = ' - Bảo hành 3 năm sản phẩm Cửa lưới sợi thủy tinh; Cửa xếp/ xếp xích lưới PL, PVC, PET; Cửa xếp lưới nhôm, lá nhựa PC.';
            worksheet.getCell('A528').value = ' - Bảo hành 5 năm sản phẩm Cửa lưới Inox.';
        }
